    ...
    // get message from sms_message table
        ...
        if(!empty($smsAttendace) && ($smsAttendace->count()>0)) {
            ...
            foreach ($attdendanceList as $attendance) {
                ...
                if ($attendance->attendacnce_type == 1) {
//
...
// process data array function send sms etc
    public  function processData($arrayData){
        ...
        if(array_key_exists("p",$arrayData)) {
            ...
            // insert data sms log
            $this->insertSmsLogData($parentList,"mobile", $arrayData['p'],4);
...
//                $smsLog->message_id = 1;
        ...
        // get sms api
        $apiGetway = SmsInstitutionGetway::select('api_path')->where('institution_id',$institute_id)->where('status',2)->first();
        ...
        // create smsSender Object
            ...
            $sendSms->sendSingleSms($batch_count,$smsApiPath,$smsMessage="attendance");
