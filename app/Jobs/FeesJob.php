...
class FeesJob implements ShouldQueue
{
    ...
    protected $studentId;
    protected $invoicePaidId;

    public function __construct($studentId,$invoicePaidId,$paidAmount)
    {
        ...
        $this->invoicePaidId = $invoicePaidId;

        // student id list log
        Log::info("Steudne Id " .$this->studentId);
        Log::info("Invoice Paid Id".$this->invoicePaidId);
        ...
        $smsFees=SmsTemplate::select('message')->where('template_name','FEES_PAID')->where('institution_id', $institute_id)->where('campus_id', $campus_id)->first();

        if(!empty($smsFees) && ($smsFees->count()>0)) {

            // get student Invoice Paid List
            $invoicePayment = InvoicePayment::where('invoice_id', $invoicePaidId)->first();

            //get student information
            $studentProfile = StudentInformation::where('id', $this->studentId)->first();

            //get fees Name
            $feesName = $invoicePayment->fees()->fee_name;
...
//         get sms api
            ...
            $sendSms->sendSingleSms($batch_count,$smsApiPath,$smsMessage="fees",$smsSender_id);
