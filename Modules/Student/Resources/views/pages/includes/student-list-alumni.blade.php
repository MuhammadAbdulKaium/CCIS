	...
	<div class="box box-solid">
		<div class="et">
			<div class="box-header with-border">
				...
				<div class="box-tools">
					<form id="w0" action="{{url("/student/manage/alumni/download/excel")}}" method="post">
		...
		<div class="box-body table-responsive">
			@if($alumniStudents->count()>0)
				<table id="example1" class="table table-striped">
					...
					@php $currentPage = $alumniStudents->currentPage(); @endphp
					{{--reset item counter--}}
					@php $i = ((($currentPage*20)-20)+1); @endphp
					{{--student enrollHistoryment looping--}}
					@foreach($alumniStudents as $enrollHistory)
						<tr>
							<td>{{$i}}</td>
							@php $studentProfile=$enrollHistory->enroll()->student();  @endphp
							<td>{{$enrollHistory->gr_no}}</td>
							<td><a href="/student/profile/personal/{{$studentProfile->id}}"> {{$studentProfile->first_name." ".$studentProfile->middle_name." ".$studentProfile->last_name}}</a></td>
							<td><a href="/student/profile/personal/{{$studentProfile->id}}">{{$studentProfile->email}}</a></td>
							...
							<td> {{$enrollHistory->batch_status}}</td>
						</tr>
						@php $i += 1; @endphp
					@endforeach
					</tbody>
				...
				<div class="text-center">
					{{ $alumniStudents->appends(Request::only([
					'search'=>'search',
					'filter'=>'filter',
