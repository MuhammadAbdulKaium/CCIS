    ...
    <div class="col-md-8">
        <div class="box box-solid">
            <div class="box-header with-border">
                ...
                <a class="btn btn-success btn-sm pull-right" href="/payroll/salary/scale/create" data-target="#globalModal" data-toggle="modal" data-modal-size="modal-lg"><i class="fa fa-plus-square" aria-hidden="true"></i>
                    Add Salary Scale</a>
            </div>
            <div class="box-body table-responsive">
                <div class="box-header"></div>
                <div class="box-body">
                    <table id="example1" class="table table-striped">
                        ...
                        <tbody>
                        <?php $i=1;?>
                        @foreach($scaleList as $item)
                            <tr>
                                <td>{{$i++}}</td>
                                <td>{{$item->scale_name}}</td>
                                <td>{{$item->gradeName->grade_name}}</td>
                                <td>{{$item->maximum_amt}}</td>
                                <td>{{$item->minimum_amt}}</td>
                                <td>
                                    <a href="/payroll/salary/scale/edit/{{$item->id}}" class="btn btn-info btn-sm" data-toggle="modal" data-target="#globalModal"><i class="fa fa-edit"></i></a>
                                    <a class="btn btn-info btn-sm" onclick="salaryGradeDelete({{$item->id}})"><i class="fa fa-trash"></i></a>
                                </td>
                            </tr>
                        @endforeach
