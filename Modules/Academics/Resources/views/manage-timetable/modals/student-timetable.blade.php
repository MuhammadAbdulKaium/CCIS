<div class="modal-header">
	...
	<h4 class="modal-title"><i class="fa fa-plus-square"></i> Class  Timetable</h4>
</div>
<div class="modal-body">
	@if($batchSectionPeriodId>0)
		@if($allClassPeriods->count()>0)
			<div class="row">
				<div class="col-sm-12">
					<p><a class="btn btn-success pull-right" href="{{url('academics/timetable/studentTimeTable/report/'.$stdId)}}">Download PDF</a></p><br/>
									...
									<td>
										...
										@if($timetableCount>0)
											...
											<div class="form-group">
												{{--<label class="control-label" for="subject_teacher">Subject</label><br/>--}}
												<span class="text-bold{{$timetableCount>0?'':'label alert-danger'}}">
												@if($timetableCount>0)
														{{$classSubjectProfile->subject_code}}
														{{--													({{$classSubjectProfile->subject_type==1?'Mandatory':'Optional'}})--}}
													@else
														N/A
													@endif
											</span>
											...
											<div class="form-group">
												{{--<label class="control-label" for="subject_teacher">Teacher</label><br/>--}}
												(<span class="text-bold {{$timetableCount>0?'':'label alert-danger'}}">
												@if($timetableCount>0)
