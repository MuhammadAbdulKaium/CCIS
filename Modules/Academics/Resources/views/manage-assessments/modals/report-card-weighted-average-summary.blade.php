...
@endphp
@if(count($semesterResultSheet)>0 && count($allSemester)>0 AND count($weightedAverageArrayList)>0)
    <div class="col-md-12">
        ...
        <form style="overflow: scroll">
            @for($i=0; $i<count($allSemester); $i++)
                @if(array_key_exists($allSemester[$i]['id'], $semesterResultSheet)==true)
                    ...
                    <table width="100%" id="grade_book_setup_table" class="table table-bordered table-striped text-center">
                        ...
                        <tr class="bg-info text-center">
                            ...
                            @if($gradeScale->assessmentsCount()>0)
                                    ...
                                    @foreach($gradeScale->assessmentCategory() as $category)
                                        ...
                                        @if($categoryWAMark>0)
                                            ...
                                            @if($assessmentList->count()>0)
                                                ...
                                                <th>
                                                    {{--category Name--}}
                                                    <input type="text" readonly class="form-control text-center" value="{{$category->name}} ({{$weightedAverageArrayList[$category->id]}})">

                                                    <input id="cate_{{$category->id}}" type="hidden" value="{{$weightedAverageArrayList[$category->id]}}">
                                ...
                                @endif
                            @endif
                            <th width="140px" id="grade_book_setup_table_head_action_{{$allSemester[$i]['id']}}">
                                <table class="table-bordered" cellpadding="10" width="100%">
                                    <tbody>
                                    <tr>
                                        {{--<td>--}}
                                        {{--<div class="input-group">--}}
                                        {{--<input type="text" readonly class="form-control text-center" value="Total">--}}
                                        {{--</div>--}}
                                        {{--</td>--}}
                                        <td>
                                            <div class="input-group">
...
<script type="text/javascript">
    var class_subjects_list, assessment_info, semester_list, semester_result_sheet;
    $(function(){

        class_subjects_list = JSON.parse('<?php echo json_encode($classSubjects);?>');
        assessment_info = JSON.parse('<?php echo json_encode($assessmentInfo);?>');
        semester_list = JSON.parse('<?php echo json_encode($allSemester);?>');
        semester_result_sheet = JSON.parse('<?php echo json_encode((array)$semesterResultSheet);?>');
...
//        $('#download_std_report_card').click(function(){
//            // std val
//            var std_id = $('#std_id').val();
//            // checking
//            if($(this).attr('href')) $(this).removeAttr('href');
//            // add attr
//            $(this).attr('href', '/academics/manage/assessments/report-card/download/'+std_id);
//        });
    ...
    function load_grade_book(class_subjects_list, data, semester_id) {
        ...
        //alert(JSON.stringify(class_subjects_list));
        ...
        for(var i=0; i<class_subjects_list.length; i++){
            ...
            // tabe row data
            tr += '<td><table width="100%" class="table-bordered" cellpadding="10"><tbody><tr><td><div class="input-group text-center"><input type="text" readonly  value="'+sub_name+'" class="form-control"></div></td></tr></tbody></table></td>';
                ...
                if(mark['cat_'+cat_id] != undefined){
                    ...
                    for(var k=1; k<=ass_count; k++){
                        ...
                        if(ass_list[myAssId] != undefined){
                            ...
                            //td += '<td style="width: 250px"><div class="input-group text-center"><input type="text" readonly value="'+ass_mark+'" class="form-control text-center mark-input"><div class="input-group-addon"> / '+ass_points+'</div></div></td>';
                        }else{
//                                td += '<td style="width: 250px"><div class="input-group text-center"><input type="text" readonly value=" - " class="form-control text-center mark-input"></div></td>';
                        }
                    }
                    ...
                    if(ass_cat_result_count[semester_id][cat_id] != undefined){
                        ...
                        // final calculation
                        category_weighted_average = parseFloat((assTotalMarksObtained/assTotalMarks)*category_assigned_mark).toFixed(2);
                        // calculate percentage
                        percentage += (isNaN(category_weighted_average)?0:(parseFloat(category_weighted_average)));

                    }else{

                        category_assigned_mark = $('#cate_'+cat_id).val();
                        category_average_mark = (parseFloat(total_ass_mark)*100)/parseFloat(total_ass_points);
                        category_weighted_average = (isNaN(category_average_mark)?0:(parseFloat((category_average_mark*category_assigned_mark)/100).toFixed(2)));
                        // calculate percentage
                        if(category_weighted_average>0) percentage += parseFloat(category_weighted_average);

                    }
//
//                        // weighted average
                    td += '<td><div style="width: 100%"  class="input-group text-center"><input type="text" readonly value="'+category_weighted_average+' / '+category_assigned_mark+'" class="form-control text-center mark-input"></div></td>';
                    ...
                    for(var zz=0; zz<ass_count; zz++){
                        ...
                        var custom_ass_points = assessment_info['point_list'][custom_ass_id];

                        td += '<td><div style="width: 100%"  class="input-group text-center"><input type="text" readonly value="-" class="form-control text-center mark-input"></div></td>';
                    }
                }
            ...
            tr += '<td><table width="100%" class="table-bordered" cellpadding="10"><tbody><tr> <td><div class="input-group"><input type="text" readonly class="form-control text-center" value="'+percentage+' %"></div></td></tr></tbody></table></td></tr>';
