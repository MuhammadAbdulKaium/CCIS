				...
				@if(array_key_exists($allSemester[$m]['id'], $semesterResultSheet)==true)
					...
					<div class="semester">
						...
						<div class="clear" style="width: 100%;">
							...
							<div id="inst-logo">
								@if($instituteInfo->logo)
									<img src="{{public_path().'/assets/users/images/'.$instituteInfo->logo}}" class="center"  style="width:80px;height:80px; margin-left: 95px">
									<br/>
									<br/>
						...
						@php
							...
							$classMeritListWithExtraMark = $classSemesterAssessmentArrayList['merit_list_with_extra_mark'];
								...
								<table id="customers" width="100%" class="text-center table" cellspacing="5">
									...
									@php
										...
										foreach ($classSubjects as $singleClassSubject){
										   ...
										   // find subject grade list
											...
											// print subject name
											if($isCountable==1){
												if($subType==3 AND array_key_exists($sub_id, $stdAddSubList) AND $stdAddSubList[$sub_id]==0){
											...
											foreach ($assessmentInfo as $headCatId=>$headAssList){
												...
												if(array_key_exists($headCatId, $assessmentResultCountInfo)==true){
													...
													// final calculation
													$catWeightedAverage = $assTotalMarks>0?(($assTotalMarksObtained/$assTotalMarks)*$catAssignedMark):0;
									...
									<tr class="text-bold calculation">
										<th colspan="{{(6+$myAssessmentCounter)}}">
											...
											<i>Highest: {{count($subHighestMarkArrayList)>0?$subHighestMarkArrayList[0]/100:' N/A'}}, </i>
											...
											@else
												{{--// print subject marks details (total, obtained, percentage letterGrade, letterGradePoints)--}}
												@php
													$semesterGradePoint = subjectGradeCalculation((int)$semesterMarksPercentage, $gradeScaleDetails);

													// mark conversion (float to integer)
	                                                $myTotalMarks = (int) round($resultObtainedMarks*100);
												...
												<i> Merit Position: {{$semesterSectionMeritPosition}} </i>
											...
											@if($assessmentTotalObtainedMarks>0)
												<tr class="text-bold calculation">
													<th colspan="{{(2+$assessmentTotalCount)}}">
														...
														{{--class Extra Highest Maraks list --}}
														@php $classExtraHighestMarkArrayList = array_unique(array_values($classMeritListWithExtraMark)); @endphp
														<i>Highest: {{count($extraHighestMarkArrayList)>0?$extraHighestMarkArrayList[0]/100:'N/A'}}, </i>
														...
														@else
															@php
																...
																// mark conversion (float to integer)
				                                                $myTotalMarksWithExtra = (int) round($semesterExtraTotalObtained*100);
				                                                // checking section merit position
				                                                ...
				                                                // checking class merit position
				                                                if(in_array($myTotalMarksWithExtra, $classExtraHighestMarkArrayList)){
				                                                    $semesterClassMeritPositionWithExtra = (array_search($myTotalMarksWithExtra, $classExtraHighestMarkArrayList)+1);
				                                                }else{
				                                                    $semesterClassMeritPositionWithExtra = 'N/A';
				                                                }

															@endphp
															<i> GPA: {{$semesterFinalGradePoint['point']}} </i>
															<i> Merit Position: {{$semesterSectionMeritPositionWithExtra}} </i>
{{--															<i> Merit Position (Class): {{$semesterClassMeritPositionWithExtra}} </i>--}}
														@endif
													</th>
												</tr>
											@endif
							...
							<div class="clear" style="width: 100%; margin-top:25px;">
								<div class="report-comments">
									<table width="100%" class="text-center table" cellpadding="1" style="line-height: 20px;">
										<tr>
											<th>Result Status </th>
											<th>{{$resultFailedSubjectCount>0?'Failed':'Passed'}}</th>
										</tr>
										<tr>
											<th>Failed Subjects </th>
											<th>{{$resultFailedSubjectCount>0?$resultFailedSubjectCount:'-'}}</th>
										</tr>
										<tr>
											<th>Working Days</th>
							...
							<div class="clear" style="width: 100%; margin-top: 170px;">
								{{--table width:77%--}}
