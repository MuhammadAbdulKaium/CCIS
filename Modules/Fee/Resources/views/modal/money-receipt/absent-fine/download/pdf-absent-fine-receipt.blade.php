...
<h5>Absent Fine Receipt</h5>
    <p>Form Date {{date('d/m/Y',strtotime($start_date))}} to {{date('d/m/Y',strtotime($end_date))}}</p>
    <p>Class: <strong>{{$absentfineReceiptArray['class']}}</strong>   Section : <strong>{{$absentfineReceiptArray['section']}}</strong></p>
</div>
...
<table id="studentListTable" class="table table-striped table-bordered">
    ...
    @foreach($absentfineReceiptList as $key=>$receipt)
        <tr class="gradeX">
            ...
            <td width="10%">{{$absentfineReceiptList[$key]['std_class']}}</td>
            <td width="5%">{{$absentfineReceiptList[$key]['std_section']}}</td>
    ...
    <tr>
        ...
        <th colspan="2">{{$absentfineReceiptArray['totalPaidamount']}}</th>
