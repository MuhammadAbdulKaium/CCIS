...
<table id="studentListTable" class="table table-striped table-bordered" style="margin-top: 20px; font-size: 12px">
    ...
    @foreach($lateFineList as $student)

        <tr class="gradeX">
            <td width="10%"><input type="checkbox" name="invoice_id[]" class="call-checkbox" value="{{$student->id}}"> </td>
            ...
            <td>{{$student->feehead()->name}}</td>
            <td>{{$student->subhead()->name}}</td>
            <td>{{$student->lateFine()->fine_amount}}</td>
            @if(!empty($student->lateFinePaid($student->id)))
               @php $totalPaidFineAmount=$student->lateFinePaid($student->id);@endphp
                @else
                @php $totalPaidFineAmount=0;  @endphp
            @endif
            @php
                $totalFineAmount=$student->lateFine()->fine_amount;
                    @endphp

            <td>{{$totalFineAmount-$totalPaidFineAmount}}</td>
            <td><a href="/fee/collection/late-fee/single/{{$student->id}}/{{$totalPaidFineAmount}}" class="btn btn-primary" data-target="#globalModal" data-toggle="modal" data-modal-size="modal-lg">Pay</a></td>
