...
@section('page-content')
    ...
    <div class="col-md-12">
        <div class="row">
            <span class="">All Student Count  <span id="st_count">0</span></span>
            <hr>
            <span class="">Batch Section Student Count <span id="bs_count">0</span></span>
            <hr>
            <div class="col-md-6">
                <form id="student_sms_form" method="post">
                    <input type="hidden" name="_token" value="{{csrf_token()}}">
                    <div class="form-group">
                        <label for="student"> <a href="javascript:void(0)" id="add_all_student" style="text-decoration: underline;"> Select All Student </a> :</label>
                        <input type="text" class="form-control" id="demo-input-local" name="student" />
                    ...
                    <div class="form-group">
                        ...
                        <div class="input-group control-group after-add-more">
                            <input type="hidden" id="bsstudent_count" name="bsstudent_count" value="0">
        ...
        <div class="modal-dialog">
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    @if(!empty($studentSmsTemplateList) && ($studentSmsTemplateList->count()>0))
                        ...
                        <table class="table">
                            ...
                            @foreach($studentSmsTemplateList as $studentSms)
                                <tr class="item">
                                    <td><button type="button" class="choose-template  btn btn-primary" />{{$studentSms->template_name}}</td>
                                    <td class="msg">{{$studentSms->message}}</td>
                                </tr>
                            @endforeach
    ...
    {{--<div class="demo btn btn-success">Load</div>--}}
...
@section('page-script')
    ...
    $('form#student_sms_form').on('submit', function (e) {

    e.preventDefault();
    $.ajax({

    url: '/communication/sms/group/student/send_sms',
    type: 'POST',
    cache: false,
    data: $('form#student_sms_form').serialize(),
    ...
    $("#add_all_student").click(function () {

    // show waiting dialog
    waitingDialog.show('Loading...');

    var st_count = parseInt($('#student_count').val());
    $.ajax({
    url: '/student/find/all',
    dataType: 'json',
    async: false,
    ...
    $("#demo-input-local").tokenInput("/student/find/all",{
    ...
    $('#student_sms_form').append(user_id);
    $('#student_sms_form').append(phone);
    $('#student_count').val(st_count+1);
    var total_student = item.student_count;

    $('#st_count').text(total_student)
    {{--alert(total_student);--}}
    },
    ...
    var total_student = item.student_count;
    $('#st_count').text(st_count-1)
    {{--alert(st_count-total_student);--}}
    ...
    var request_url='/student/find/student/'+batch+'/'+section;
    {{--alert(request_url);--}}
    var bs_count = parseInt($('#bsstudent_count').val());
    $.ajax({
    url: request_url,
    dataType: 'json',
    async: false,
    ...
    if(data.status==false) {
        swal("Alert", "Student Not Found", "error");
    } else {
        $.each(data, function( index, value ) {
        // add value
    ...
    /// Token Input and get all class and section Data;
    $("#demo-input-locals").tokenInput("/student/find/all",{
    preventDuplicates: true,
    searchDelay: 200,
    ...
    $('#student_sms_form').append(user_id);
    $('#student_sms_form').append(phone);
    $('#bsstudent_count').val(bs_count+1);
    var total_student = item.student_count;
