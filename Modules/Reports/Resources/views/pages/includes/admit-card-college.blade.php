...
<div class="col-md-10 col-md-offset-1">
	...
	@if(count($classSubjectList)>0)
		...
		<form id="std_admit_card_form" action="{{url('/reports/admit-card/download/college')}}" method="POST" target="_blank">
			...
			<table class="table table-striped table-bordered sorted_table text-center" role="grid" aria-describedby="example2_info">
				...
				{{--subject counter--}}
				@php $myIndex = 1; @endphp
				{{--student enrollment looping--}}
				@foreach($classSubjectList as $groupId=>$groupDetails)
					...
					<tr class="item" id="{{$myIndex}}">
						...
						<td>
							{{implode(", ", $subCodes)}}
							<input id="{{$myIndex}}_sub_code" type="hidden" name="routine[{{$myIndex}}][sub_code]" value="{{implode(", ", $subCodes)}}">
						</td>

						<td>
							{{$groupDetails->name}}
							<input id="{{$myIndex}}_sub_id  " type="hidden" name="routine[{{$myIndex}}][sub_id]" value="{{$groupId}}">
							<input id="{{$myIndex}}_sub_name" type="hidden" name="routine[{{$myIndex}}][sub_name]" value="{{$groupDetails->name}}">
							<input id="{{$myIndex}}_sub_type" type="hidden" name="routine[{{$myIndex}}][sub_type]" value="{{$groupDetails->type}}">
						</td>
						<td>
