                            ...
                            <div class="row section-seven">
                                <div class="col-md-12 col-xs-12 attendance-details">
                                    <div class="table-overflow table-responsive">
                                        <h3 class="atten-std-info atten-std-details font16">
                                            {{$reportDetails->class_name.' ('.$reportDetails->section_name.')'}} Contact Report</h3>

                                        <table class="table table-bordered table-hover text-center" style="font-size: 12px; color: black">
                                            <thead>
                                            <tr>
                                                <th width="2%" >#</th>
                                                <th width="3%" >Roll</th>
                                            ...
                                            @foreach($studentList as $index=>$std)
                                                {{--address--}}
                                                @php
                                                    $present = $std->user()->singleAddress("STUDENT_PRESENT_ADDRESS");
                                                    $permanent = $std->user()->singleAddress("STUDENT_PERMANENT_ADDRESS");
                                                @endphp
                                                ...
                                                <tr>
                                                    ...
                                                    <td>{{$std->email}}</td>
                                                    <td>{{$std->student()->phone}}</td>
                                                    {{--<td>Ezarkandi,Kalapaharia,Araihazar,Narayan Gonj</td>--}}
                                                    {{--<td>Ezarkandi,Kalapaharia,Araihazar,Narayan Gonj</td>--}}
                                                    <td>{{$present?$present->address:''}}</td>
                                                    <td>{{$permanent?$permanent->address:''}}</td>
                                                    {{--<td>{{$permanent?(string)$permanent->address:'-'}}</td>--}}
                                                </tr>
                                            @endforeach
                                            </tbody>
                                        </table>
