...
<div class="col-sm-12">
    @foreach($studentList as $studentProfile)
        ...
        >
            ...
            <div class="row">
                <div class="col-xs-12">
                    <img src="{{url('assets/users/images/'.getInstituteProfile()->logo)}}" style="position: absolute" height="60px;" width="60px">
                    <div class="heading">
            ...
            <div class="row invoice-info" style="margin-top: 15px; margin-left: 2px; margin-right: 2px;">
        ...
        @php $count = count($batchSemseterList);
        @endphp
        @php $countSemester=0; @endphp
        @foreach($batchSemseterList as $bachSemster)

                @php $countSemester++ @endphp
                @if($countSemester%5==0)
                    ...
                    <div class="row">
                        <div class="col-xs-12 table-responsive">
                            <table class="table feeheadTable  table-bordered">
                                <thead>
                                <tr>
                                    <th colspan="5" style="text-align: center; font-weight: bold">{{$bachSemster->semester()->name}}</th>
                                </tr>
                                <tr>
                                ...
                                @foreach($classSubjectList as $key=>$subject)
                                    ...
                                    {{--checking subject type--}}
                                    @if($subType==1 || in_array($key, $myAdditionalSubList))
                                        @if(array_key_exists(1, $subject['code']))
                                        <tr>
                                            <td> </td>
                                            {{--                        <td>{{dd($subject)}}</td>--}}
                                            <td>{{$subject['name']}}</td>
