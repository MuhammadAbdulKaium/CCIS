...
@if(!empty($studentWaivers))
    ...
    @foreach($batchSectionList as $batchSection)
        @php  $batchSectionShorting = sectionSorter($batchSection->section, batchSorter($batchSection->batch, $studentWaivers)); @endphp

        <div class="" style="clear: both; padding-bottom: 20px;">
            <div id="batch">
                <h4>Class : {{getBatchName($batchSection->batch)}} </h4>
            </div>
            <div id="section">
...
<table id="tableDesign" class="transactions_table" style="margin-top: 20px;">
        ...
        <tbody>
        @foreach($batchSectionShorting as $studentWaiver)

            <tr>
                <td>{{$studentWaiver->std_id}}</td>
