                            ...
                            @foreach($total_Day_Date as $key=>$date)
                                @if($days[$i] == $key)
                                    ...
                                    <tr class="gradeX">
                                        ...
                                        <td style="text-align: center;">
                                            @if(isset($scheduledData[$subjectProfile->id][22]))    
                                                {{ $scheduledData[$subjectProfile->id][22] }}
                                            @else
                                            {{ '--' }}
                                            @endif
                                        ...
                                        <td style="color: green;">
                                            ...
                                            @if(isset($scheduledData) && $timetableCount>0)
                                                ...
                                                @if(isset($scheduledData[$subjectProfile->id][0]) && $scheduledData[$subjectProfile->id][11] == 2 && $scheduledData[$subjectProfile->id][9] == $date)
                                                    ...
                                                    @if($currentTime >= $scheduledData[$subjectProfile->id][19] && $scheduledData[$subjectProfile->id][20] <= $currentTime)
    ...
    $.ajax({
        ...
        success:function(data){
            ...
            if(data.tabActive == 'onlineclass'){
                ...
                window.location.replace("http://127.0.0.1:8000/onlineacademics/onlineacademic/LiveClassScheduled/"+ScheduleId);
            }   
        },
        error:function(){
            // sweet alert
