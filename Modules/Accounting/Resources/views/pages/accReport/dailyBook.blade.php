...
@section('content')
                                ...
                                <div class="col-md-4">
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <input list="myList" type="text" class="form-control" id="voucherTypelist">
                                        </div>
                                    </div>
                                    <datalist id="myList">
    ...
    <script type = "text/javascript">
        ...
        function searchDataLoad() {
            ...
            var token = "{{ csrf_token() }}";
            var voucherType = $('#voucherTypelist').val();
            var fromDate = $('#fromDate').val();
            var toDate = $('#toDate').val();
            var dataSet = '_token='+token+'&fromDate='+fromDate+'&toDate='+toDate+'&voucherType='+voucherType;

            if(chk == 0){
                $.ajax({
                    url: "{{ url('accounting/accreport/accdailybook') }}",
                    type: 'post',
                    data: dataSet,
                    beforeSend: function () {
                    }, success: function (data) {
        ...
        function clearFormData() {
            $('#fromDate,#toDate,#voucherTypelist').val('');
        }
        ...
        function excelDownload() {
            ...
            var gap = '_____';
            if(voucherType.length == 0){ voucherType = gap;};
            if(fromDate.length == 0){ fromDate = gap; };
            if(toDate.length == 0){ toDate = gap;};

            var myUrl = '{{url('accounting/accreport/accdailybookexcel/')}}';
            myUrl = myUrl+'/'+voucherType+'/'+fromDate+'/'+toDate;
