	...
	<div class="box box-solid">
		<div class="et">
			<div class="box-header with-border">
				<h3 class="box-title"><i class="fa fa-user"></i> User (Employee) Assignment</h3>
			</div>
		</div>
		<div class="box-body">

			@php $employeeInfo = $userProfile->employee(); @endphp
			...
			<table class="table table-bordered table-striped text-center">
				...
				<tr>
					...
					<td>
						<a href="/employee/profile/personal/{{$employeeInfo->id}}">{{$employeeInfo->title." ". $employeeInfo->first_name." ".$employeeInfo->middle_name." ".$employeeInfo->last_name}}</a>
						({{$employeeInfo->alias}})
					</td>
					<td>{{$userProfile->email}}</td>
					<td>{{$employeeInfo->gender}}</td>
					<td>{{$employeeInfo->department()->name}}</td>
