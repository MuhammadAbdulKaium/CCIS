            ...
            <section class="content">
                ...
                <div class="box box-solid">
                    <div class="box-header with-border">
                        ...
                        <div class="box-tools">
                            <a v-if="dataList.pageAccessData['accounts/receive-voucher-data/create']" class="btn btn-success btn-sm" @click="openModal('addForm', 'receive-voucher-data/create',true)"><i class="fa fa-plus-square"></i> New</a>
                        </div>
                    </div>
                    <div class="box-body">
                                ...
                                <tbody>
                                    <template v-if="Object.keys(paginate_data).length > 0">
                                        <tr v-for="(list, index) in paginate_data" v-bind:key="index">
                                            ...
                                            <td>@{{list.remarks}}</td>
                                            <td>
                                                <template v-if="dataList.pageAccessData['accounts/receive-voucher.edit'] || dataList.pageAccessData['accounts/receive-voucher.delete'] || dataList.pageAccessData['accounts/receive-voucher.show'] || dataList.pageAccessData['accounts/receive-voucher.approval']">
                                                    <a v-if="list.has_approval=='yes' && dataList.pageAccessData['accounts/receive-voucher.approval']" class="btn btn-primary btn-xs" @click="voucherApproval('receive-voucher-approval',list.id)"
                                                    ><i class="fa fa-check-square" aria-hidden="true"></i> Approve</a>
                                                    <a v-if="list.status==0 && !list.someOneApproved && dataList.pageAccessData['accounts/receive-voucher.edit']" class="btn btn-primary btn-xs" @click="openModal('addForm', 'receive-voucher-data/'+list.id+'/edit')" title="Edit"><i class="fa fa-edit"></i></a>
                                                    <a v-if="list.status==0 && dataList.pageAccessData['accounts/receive-voucher.delete']"  @click="deleteItem(list.id)"
                                                        class="btn btn-danger btn-xs" data-placement="top"
                                                        data-content="delete" title="delete"><i class="fa fa-trash-o"></i></a>
                                                    <a v-if="dataList.pageAccessData['accounts/receive-voucher.show']" class="btn btn-primary btn-xs" @click="openModal('detailsForm', 'receive-voucher-data/'+list.id)" 
