...
<form action="{{url('/accounts/budget-allocation/update-budget/'.$budget->id)}}" method="POST">
    @csrf

    <div class="modal-header">
        <button aria-label="Close" data-dismiss="modal" class="close" type="button"><span aria-hidden="true">×</span></button>
        <h4 class="modal-title text-bold">
            <i class="fa fa-plus-square"></i> Edit Budget
        </h4>
    </div>
    <!--modal-header-->
    <div class="modal-body">
        <div class="row">
            ...
            <div class="col-sm-4">
                <input type="text" id="year" class="form-control hasDatepicker"
                       ...
                       size="10" value="{{$budget->fiscal_year}}" required>
            </div>
            <div class="col-sm-2">
                    ...
                    @foreach($ledgerParents as $ledgerParent)
                        ...
                        <optgroup label="{{$ledgerParent->account_name}}">
                            @foreach($selectedLedgers as $ledger)
                                <option value="{{$ledger->id}}" {{($budget->account_id == $ledger->id)?'selected':''}}>[{{$ledger->account_code}}] {{$ledger->account_name}}</option>
                            @endforeach
                        </optgroup>
                    @endforeach
                </select>
        ...
        <div class="row" style="margin-top: 20px">
            ...
            <div class="col-sm-4">
                <input type="number" name="amount" class="form-control budget-field" value="{{$budget->amount}}" required>
            </div>
        </div>
        ...
        <div class="row" style="margin-top: 10px">
            @while(true)
                ...
                <div class="col-sm-4" style="margin-top: 20px">
                    <input type="number" name="months[{{$i}}]" class="form-control month-field" value="{{$month->amount}}">
                </div>
...
<script>
    $(document).ready(function (){
        ...
        $(document).on('keydown', '.budget-field', function (){
