    ...
    public function index(Request $request)
    {
        ...
        if(count($paginate_data)>0){
            ...
            foreach ($paginate_data as $v){
                $approval_info = self::getApprovalInfo('payment_voucher', $v);
                $approval_access = $approval_info['approval_access'];
                $lastStep = $approval_info['last_step'];
                $v->someOneApproved = self::someOneApproved('payment_voucher', $v->id);
                ...
                if (!empty($id)) {
                    ...
                    if ($flag) {
                        ...
                        ApprovalNotification::create([
                            ...
                            'unique_name' => 'payment_voucher',
                            'menu_link' => 'accounts/payment-voucher',
        ...
        try {
            ...
            if (!empty($approvalData)) {
                if ($approvalData->status == 0) {
                    ...
                    $approval_info = self::getApprovalInfo('payment_voucher', $approvalData);
                    $step = $approvalData->approval_level;
                    $approval_access = $approval_info['approval_access'];
                    $last_step = $approval_info['last_step'];
                    if($approval_access){
                        $approvalLayerPassed = self::approvalLayerPassed('payment_voucher', $approvalData, true);
                        ...
                        if ($approvalLayerPassed) {
                            ...
                            if($step==$last_step){
                                ...
                                $approvalHistoryInfo = self::generateApprovalHistoryInfo('payment_voucher', $approvalData);
                                ApprovalNotification::where([
                                    'unique_name' => 'payment_voucher',
                                    'menu_id' => $id,
                                    'action_status' => 0,
