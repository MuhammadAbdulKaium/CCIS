    ...
    public function index(Request $request)
    {
        ...
        if(count($paginate_data)>0){
            ...
            foreach ($paginate_data as $v){
                $approval_info = self::getApprovalInfo('contra_voucher', $v);
                $approval_access = $approval_info['approval_access'];
                $lastStep = $approval_info['last_step'];
                $v->someOneApproved = self::someOneApproved('contra_voucher', $v->id);
                ...
                if (!empty($id)) {
                    ...
                    if ($flag) {
                        ...
                        ApprovalNotification::create([
                            ...
                            'unique_name' => 'contra_voucher',
                            'menu_link' => 'accounts/contra-voucher',
        ...
        try {
            ...
            if (!empty($approvalData)) {
                if ($approvalData->status == 0) {
                    ...
                    $approval_info = self::getApprovalInfo('contra_voucher', $approvalData);
                    $step = $approvalData->approval_level;
                    $approval_access = $approval_info['approval_access'];
                    $last_step = $approval_info['last_step'];
                    if($approval_access){
                        $approvalLayerPassed = self::approvalLayerPassed('contra_voucher', $approvalData, true);
                        ...
                        if ($approvalLayerPassed) {
                            ...
                            if($step==$last_step){
                                ...
                                $approvalHistoryInfo = self::generateApprovalHistoryInfo('contra_voucher', $approvalData);
                                ApprovalNotification::where([
                                    'unique_name' => 'contra_voucher',
                                    'menu_id' => $id,
                                    'action_status' => 0,
                        ...
                        DB::commit();
                        $output = ['status'=>1, 'message'=>'Contra voucher successfully approved'];
                    ...
                    if($approvalData->status==2){
                        $output = ['status'=>0, 'message'=>'Contra voucher already reject'];
                    }else{
                        $output = ['status'=>0, 'message'=>'Contra voucher already approved'];  
                    }
                }
            }else{   // end if(!empty($approvalData))
                $output = ['status'=>0, 'message'=>'Contra Voucher not found'];
            }
        } catch (Throwable $e) {
            DB::rollback();
            throw $e;
