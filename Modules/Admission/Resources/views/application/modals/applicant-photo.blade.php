...
<div class="modal-header">
	...
	<h4 class="modal-title"><i class="fa fa-picture-o"></i> {{$profilePhoto?'Update':'Upload'}} Photo</h4>
</div>
{{--form statrs here--}}
<form id="stu-photo-form" action="{{url('/admission/applicant/photo/update')}}" method="POST" enctype="multipart/form-data">
	<input type="hidden" name="_token" value="{{csrf_token()}}">
	<input type="hidden" name="applicant_id" value="{{$applicantProfile->id}}">
	<input type="hidden" name="document_id" value="{{$profilePhoto?$profilePhoto->id:'0'}}">
	<div class="modal-body">
		<div class="row stu-photo-form text-center">
			<div class="col-md-12">
				<img id="stu-photo-prev" src="{{URL::asset($profilePhoto?$profilePhoto->doc_path.'/'.$profilePhoto->doc_name:'assets/users/images/user-default.png')}}" alt="No Image" width="150px" height="150px">
			</div>
			<div class="col-md-10 col-sm-offset-1">
				<br>
				<div class="form-group image">
					<input name="image" value="" type="hidden">
					<input id="image" name="image" title="Browse Photo" accept="image/*" data-filename-placement="inside" onchange="uploadImage(this)" data-target="#stu-photo-prev" type="file" required>
